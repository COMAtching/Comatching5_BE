...
### DLT 재적재 — 고친 뒤 되돌리는 경로

알림을 받고 원인을 고친 다음의 마지막 조각이다. 소비를 선언한 각 서비스에
내부 엔드포인트가 붙는다(`/api/internal/**` 은 게이트웨이가 라우팅하지 않고
`X-Internal-Token` 으로 보호된다):
...
`KafkaDltRedriveService` 가 전용 그룹으로 `<토픽>.DLT` 를 커밋 오프셋부터
읽어 **원본 토픽에 키·값 그대로 재발행**하고 커밋한다. 설계 결정들:

- **리스너 직접 호출이 아니라 원본 토픽 재발행** — 평소의 소비 경로
  (재시도 → 실패 시 DLT → 알림)를 그대로 다시 탄다. 안 고쳐진 메시지는
  다시 DLT 로 돌아와 다시 알림이 오므로 재처리 전용 실패 경로가 필요 없다.
- **끝 오프셋 스냅샷** — 시작 시점의 끝까지만 처리한다. 재발행분이 곧바로
  다시 실패해 DLT 로 돌아오는 경우, 스냅샷이 없으면 같은 메시지를 무한히
  쫓아간다.
- **재발행 확인 후 커밋 (at-least-once)** — 커밋 직전에 죽으면 다음 호출이
  한 번 더 되돌리지만 컨슈머가 멱등이라 안전하다. 반대 순서는 유실이다.
- **진단 헤더 제거** — `kafka_dlt-*`·`kafka_exception*` 헤더를 걷어내고
  재발행한다. 남기면 실패를 반복할 때마다 스택트레이스가 누적된다.
  `__TypeId__` 같은 역직렬화 헤더는 보존한다.
- **허용 목록 검증** — `comatching.kafka.dlt-topics` 에 선언된 토픽만
  받는다. 오타로 임의 토픽에 컨슈머가 붙는 것을 막는다.
- **자동 재적재는 두지 않는다** — DLT 는 "재시도로 회복 안 된" 실패라 원인
  교정 없이 되돌리면 재시도-DLT-재적재 무한 순환이 된다. 사람이 고쳤다고
  판단한 시점에만 트리거한다.

**운영 제약 재확인**: `profile-updates` 재적재는 tombstone TTL(2일) 안에.
재발행은 현재 트래픽과 순서가 섞이므로 같은 회원의 더 새로운 이벤트를 옛
이벤트가 덮어쓸 수 있다("남은 한계"의 버전 부재 항목).
...
| 원인 수정 후 재적재하면 평소 소비 경로로 재처리되고 키가 보존됨 | EmbeddedKafka, 적재→수정→재적재 전체 사이클 (`KafkaDltRedriveIT`) | ✅ |
| 재적재는 커밋 오프셋을 남겨 중복 재적재하지 않고, 미선언 토픽은 거부함 | 〃 | ✅ |
...
1. **메일 발송은 재시도가 중복 발송이 될 수 있다** — 후보 삭제·갱신은 멱등이라
